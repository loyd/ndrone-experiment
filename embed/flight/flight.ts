...
gyro.stream('velocity', 1e3/gyro.rate, (err, wx, wy, wz, dt) => {
    if(err) throw err;
    raw[0] += wx * dt;
    raw[1] += wy * dt;
    raw[2] += wz * dt;
...
accel.stream('acceleration', 1e3/accel.rate, (err, ax, ay, az) => {
    if(err) throw err;
    raw[3] = ax;
    raw[4] = ay;
    raw[5] = az;
...
magn.stream('induction', 1e3/magn.rate, (err, mx, my, mz) => {
    if(err) throw err;
    raw[6] = mx;
    raw[7] = my;
    raw[8] = mz;
